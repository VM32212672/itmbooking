...
from reservationApp.models import Booking, Category, Location, Bus, Schedule,Drive
...
def scheduled_trips(request):
    if not request.method == 'POST':
        ...
        date = datetime.strptime(request.POST['date']).date()
        year = date.strftime('%Y')
        month = date.strftime('%m')
        day = date.strftime('%d')
        drive=Drive.objects.get(id = request.POST['drive'])
